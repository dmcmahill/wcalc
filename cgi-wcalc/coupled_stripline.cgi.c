/* $Id: coupled_stripline.cgi.c,v 1.2 2006/03/01 15:27:38 dan Exp $ */
...
/*
 * a cgi interface to the coupled_stripline calculator
 */
...
#include "coupled_stripline_id.h"
...
static const char *name_string="coupled_stripline.cgi";
...
int cgiMain(void){
   ...
   * the coupled_stripline structure
   */
...
#ifdef DEBUG
    printf("coupled_stripline.cgi:  checking for a cookie to load\n");
#endif
    /* load a stored cookie if it exists */
    if(cgiCookieStringNoNewlines(name_string, cookie_load_str, COOKIE_MAX) ==
       cgiCookieSuccess) {
#ifdef DEBUG
    printf("coupled_stripline.cgi:  loading cookie \"%s\"\n", cookie_load_str);
#endif
      coupled_stripline_load_string(line, cookie_load_str);
#ifdef DEBUG
    printf("coupled_stripline.cgi:  finished loading cookie\n");
#endif
    }
  ...
  if (!input_err){
    cookie_str = coupled_stripline_save_string(line);
  ...
  case ANALYZE:
    /* 
     * in case coupled_stripline_calc has some error output, surround it
...
#include "coupled_stripline_html.c"
