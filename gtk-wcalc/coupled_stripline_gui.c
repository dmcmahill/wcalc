/* $Id: coupled_stripline_gui.c,v 1.8 2009/02/05 22:15:21 dan Exp $ */
...
static void calculate( coupled_stripline_gui *gui, GtkWidget *w, gpointer data );
static void update_display( coupled_stripline_gui *gui);
...
{
  coupled_stripline_gui *new_gui;
  Wcalc *wcalc;

  /* allocate memory */
  new_gui = (coupled_stripline_gui *) malloc(sizeof(coupled_stripline_gui));
  if(new_gui == NULL)
    {
      fprintf(stderr,"coupled_stripline_gui_new: malloc() failed\n");
      exit(1);
    }
  ...
  /* create the coupled_stripline line which will be used */
  new_gui->line = coupled_stripline_line_new();
...
void coupled_stripline_gui_init(Wcalc *wcalc, GtkWidget *main_vbox,FILE *fp)
{
  ...
  if (fp != NULL){
#ifdef DEBUG
    g_print("coupled_stripline_gui.c:coupled_stripline_gui_init():  calling coupled_stripline_load\n");
#endif
    if (coupled_stripline_load(gui->line,fp) != 0) {
      alert("ERROR:  Could not load the coupled_stripline\n"
...
#ifdef DEBUG
    g_print("coupled_stripline_gui.c:coupled_stripline_gui_init():  new using defaults\n");
#endif
  }
...
static void analyze( GtkWidget *w, gpointer data )
{
  calculate(WC_COUPLED_STRIPLINE_GUI(data), w, "analyze");
}

static void synthesize( GtkWidget *w, gpointer data )
{
  calculate(WC_COUPLED_STRIPLINE_GUI(data), w, "synthesize");
}
...
#ifdef DEBUG
  g_print("coupled_stripline_gui.c:calculate():  freq = %g %s\n",
	  gui->line->freq/gui->units_freq->sf, gui->units_freq->name);
#endif

#ifdef DEBUG
  g_print("coupled_stripline_gui.c:calculate():  calculation with \"%s\"\n",
	  (char *) data);
#endif

  /* XXX should use an enum and switch... */
  if( strcmp(data,"analyze")==0) {
    rslt=coupled_stripline_calc(gui->line, gui->line->freq);
  }
  else if( strcmp(data,"synthesize")==0) {
    rslt=coupled_stripline_syn(gui->line, gui->line->freq);
  } else{
    fprintf(stderr,"coupled_stripline_gui.c:  error in coupled_stripline callback\n"
...
static void use_z0k_pressed(GtkWidget *widget, gpointer data )
{
  ...
  gui = WC_COUPLED_STRIPLINE_GUI(data);
  gtk_widget_set_sensitive (gui->text_z0, TRUE);
  gtk_widget_set_sensitive (gui->text_k, TRUE);
  gtk_widget_set_sensitive (gui->text_z0e, FALSE);
  gtk_widget_set_sensitive (gui->text_z0o, FALSE);
...
{
#ifdef DEBUG
  g_print("coupled_stripline_gui.c:gui_save():  wcalc=%p, fp=%p, name=%p\n",
	  wcalc,fp,name);
#endif
