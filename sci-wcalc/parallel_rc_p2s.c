...
/*
 * [Cs, Rs, Qs, Cp, Rp, Qp] = parallel_rc_p2s(Cp, Rp, Qp, freq, flag)
 */
...
{
  /* inputs */
  double *Cp, *Rp, *Qp, *freq, *flag;

  unsigned int *ind_Cp,*ind_Rp,*ind_Qp,*ind_freq,*ind_flag;
  ...
  if (nrhs == 4) 
    ...
    {
      mexErrMsgTxt("wrong number of input arguments to PARALLEL_RC_P2S"
		   " (needs 4 or 5).");
    } 

  if (nlhs > 6)
    {
      mexErrMsgTxt("wrong number of output arguments to PARALLEL_RC_P2S"
		   " (needs <= 6).");
    }
   ...
   *              pointer)
   */
  CHECK_INPUT(CP_IN, CP, ind_Cp, Cp);
  ...
  if(has_flag){
    ...
    if ( (flag = malloc(sizeof(double))) == NULL ) {
      fprintf(stderr,"parallel_rc_p2s.c:  malloc() failed\n");
      exit(1);
    }
    *flag=0;
    ind_flag=&fixed;
