/* $Id: coupled_stripline_calc.c,v 1.2 2006/02/13 19:20:23 dan Exp $ */

static char vcid[] = "$Id: coupled_stripline_calc.c,v 1.2 2006/02/13 19:20:23 dan Exp $";
...
if (!mxIsNumeric(x) ||                                                \
    ...
    mexErrMsgTxt(#y " must be a real valued vector "                  \
                    "in COUPLED_STRIPLINE_CALC.");                   \
}                                                                     \
if (vector && (m>1 || n>1) &&                                         \
    ( (m != rows) || (n != cols) ) ){                                 \
    mexErrMsgTxt(#y ": all vector/matrix inputs must have the same"   \
                    " dimensions in COUPLED_STRIPLINE_CALC.");       \
    (z) = NULL;                                                       \
}                                                                     \
else if(m*n > 1) {                                                    \
 vector=1;                                                            \
		 ...
		 )
{
  ...
  if (nrhs != 10) {
    mexErrMsgTxt("wrong number of input arguments to COUPLED_STRIPLINE_CALC"
		 " (needs 10).");
  } 

  if (nlhs > 10) {
    mexErrMsgTxt("wrong number of output arguments to COUPLED_STRIPLINE_CALC"
		 " (needs <= 10).");
  }
